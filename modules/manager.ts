...
async function getHouseStats() {
  ...
  const points = houseMap.map((shorthand, roleId) => {
    ...
    return {
      ...
      emoji: u.sf.emoji.houses[shorthand as keyof typeof u.sf.emoji.houses],
      shorthand
    };
  });
...
async function slashManagerRankHouseEmber(interaction: Augur.GuildInteraction<"CommandSlash">) {
  try {
...
async function getMopBucketWinner(client: Discord.Client, time?: string) {
...
async function rankReset(client: Discord.Client<true>, dist = 10_000) {
...
async function slashManagerRankReset(int: Augur.GuildInteraction<"CommandSlash">) {
  try {
