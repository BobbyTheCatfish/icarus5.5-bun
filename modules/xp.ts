...
async function rankClockwork(client: Discord.Client) {
  ...
  for (const user of response.users) {
    ...
    try {
      if (!member.roles.cache.has(u.sf.roles.moderation.trusted) && !banned.features.xp.trustedAlerts.includes(user.discordId)) {
        let content;
...
Module.setUnload(() => active)
  ...
  .addEvent("messageUpdate", async (msg, newMsg) => {
    ...
    const answers = await Promise.all(newMsg.poll.answers.map((s: Discord.PollAnswer) => s.fetchVoters() as Promise<Discord.Collection<string, Discord.User>>));
