...
const retract = new u.MessageActionRow().setComponents([
  new u.Button().setCustomId("modCardRetract").setEmoji("⏪").setLabel("Retract").setStyle(ButtonStyle.Danger)
]);

function userBackup(person: Discord.GuildMember | Discord.User | Discord.Webhook) {
  return `${person} (${u.escapeText("displayName" in person ? person.displayName : person.name)})`;
}

const messageFromMods = "## 🚨 Message from the LDSG Mods:\n";
const code = "[Code of Conduct](http://ldsgamers.com/code-of-conduct)";

function logEmbed(int: Augur.GuildInteraction<"Any"> | Discord.Message, tg: Discord.GuildMember | Discord.User) {
  return u.embed({ author: tg })
    .addFields(
...
function blocked(user: Discord.GuildMember | Discord.User) {
...
function compareRoles(mod: Discord.GuildMember, target: Discord.GuildMember) {
...
type FlagInfo = {
  msg?: Discord.Message;
  member: Discord.GuildMember | Discord.User | Discord.Webhook;
  matches?: string | string[];
  pingMods?: boolean;
  snitch?: Discord.GuildMember | Discord.User;
  flagReason: string;
  furtherInfo?: string;
}
...
const modCommon = {
  ...
  note: async function(interaction: Augur.GuildInteraction<"CommandSlash" | "Modal">, target: Discord.GuildMember | Discord.User, note: string) {
    let success = false;
    try {
  ...
  office: async function(interaction: Augur.GuildInteraction<"CommandSlash">, target: Discord.GuildMember, reason: string, apply = true) {
    let success = false;
    try {
  ...
  rename: async function(interaction: Augur.GuildInteraction<"CommandSlash" | "Modal">, target: Discord.GuildMember, newNick: string, reset = false) {
    let success = false;
    try {
  ...
  trust: async function(interaction: Augur.GuildInteraction<"CommandSlash" | "SelectMenuString" | "Button">, target: Discord.GuildMember, apply = true) {
    let success = false;
    try {
  ...
  /** Give someone the Trusted+ Role */
  trustPlus: async function(interaction: Augur.GuildInteraction<"CommandSlash" | "SelectMenuString">, target: Discord.GuildMember, apply = true) {
    let success = false;
    try {
  ...
  watch: async function(interaction: Discord.Interaction<"cached"> | Discord.Message, target: Discord.GuildMember | string, apply: boolean = true) {
    let success = true;
    try {
